...
export async function getTraderClosedStats(
  ...
  for (let page = 0; page < MAX_PAGES; page++) {
    ...
    const res = await fetch(url)
    if (!res.ok) break
    const rows = parseJsonArray<ClosedPosition>(await res.json())
    if (rows.length === 0) break
    for (const p of rows) {
      const pnl = p.realizedPnl ?? 0
      lifetimeRealizedPnl += pnl
      closedCapitalContributed += closedPositionCapitalUsd(p)
      if (p.conditionId) markets.add(String(p.conditionId))
      const tsMs = (p.timestamp ?? 0) * 1000
      if (tsMs >= cutoff) recentRealizedPnl += pnl
      if (pnl > 0) wins++
    ...
    if (rows.length === PAGE && page === MAX_PAGES - 1) truncated = true
  }
  ...
  return {
    lifetimeRealizedPnl,
    recentRealizedPnl,
    closedPositionsSampled: sampled,
